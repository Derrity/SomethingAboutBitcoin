...
void sendRequest(std::ofstream &file, const std::string &key) {
    while (true) {
        try {
            ...
            if (curl) {
                curl_easy_setopt(curl, CURLOPT_URL,
                                 ("https://api.blockchair.com/bitcoin/dashboards/address/" + address + "?key=" +
                                  key).c_str());
                ...
                if (res != CURLE_OK) {
                    ...
                    int final_balance = j["data"][address]["address"]["balance_usd"];
                    if (final_balance > 0) {
                        successCount++;
...
int main(int argc, char *argv[]) {
    ...
    std::cin >> threadCount;
    std::cout << "Enter the API Key of Blockchair: ";
    std::cin >> key;
    std::vector<std::future<void>> futures;
    std::ofstream file("addresses.txt", std::ios::app);
    for (int i = 0; i < threadCount; i++) {
        futures.push_back(std::async(std::launch::async, sendRequest, std::ref(file), key.c_str()));
    }
    for (auto &f: futures) {
        f.get();
    }
